...
class NotificationService {
    ...
    sendNotificationToSession(sessionId: number, notification: any, excludeUserId?: number) {
    ...
    async notifyCollaborationRequest(sessionId: number, fromUserId: number, toUserId: number, sessionName: string) {
    ...
    async notifyRequestResponse(sessionId: number, fromUserId: number, toUserId: number, status: "accepted" | "rejected", sessionName: string) {
        const fromUser = await storage.getUser(fromUserId);
        const username = fromUser?.username || `User ${fromUserId}`;
    ...
    async notifyUserJoined(sessionId: number, userId: number, sessionName: string) {
